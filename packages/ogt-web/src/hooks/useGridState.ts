...
function pruneSummits(
  ...
  return summits.map((row, i) =>
    row.map((s, j) => ({
      connector_angle:
        eligibility.connectors[i]?.[j] ? s.connector_angle : null,
...
export function useGridState(initialRows: number, initialCols: number) {
    ...
    setSummits((prev) => {
      ...
      for (let i = 0; i <= rows; i++) {
        for (let j = 0; j <= cols; j++) {
          if (eligibility.connectors[i]?.[j] && next[i][j].connector_angle === null) {
            next[i][j].connector_angle = computeConnectorDirection(tiles, i, j)
          }
        }
      }
  ...
  const clearAllConnectors = useCallback(() => {
    setSummits((prev) =>
      prev.map((row) =>
        row.map((s) => ({ ...s, connector_angle: null })),
      ),
    )
  }, [])
  ...
  const clearAllChamfers = useCallback(() => {
    setSummits((prev) =>
      prev.map((row) =>
        row.map((s) => ({ ...s, tile_chamfer: false })),
      ),
    )
  }, [])
